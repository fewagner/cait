...
The ``tests`` folder contains test files that can be automatically executed using ``pytest`` within the repository. Notice that files and functions (classes) have to start with ``test_`` to be included automatically.

The ``docs`` folder contains the documentation, written in restructured text, automatically generated with ``sphinx`` according to the ``.readthedocs.yaml`` file. The tutorials subfolder contains tutorials in ``jupyter`` notebooks, that are automatically rendered into the documentation.

The ``dist`` folder contains the wheel files, that are used for installation. 

The ``.gitignore`` file excludes all files that are too large for git or specific for each user: data, virtual environment, ...

The ``pyproject.toml`` file contains information for the installation of the package: dependencies, version number, ...

The ``.gitlab-ci.yml`` contains instructions that are automatically run on gitlab (like tests or building containers). The ``Dockerfile`` is related to that.
