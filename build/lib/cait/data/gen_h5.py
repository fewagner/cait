...
def gen_dataset_from_rdt(path,
    ...
    if 0.0 in tpa_list:
        ...
        with Pool(processes) as p:  # basically a for loop running on 4 processes
            p_mainpar_list_event = p.map(calc_main_parameters, pulse_event[0, :, :])
            l_mainpar_list_event = p.map(calc_main_parameters, pulse_event[1, :, :])
        ...
        if calc_fit:
            with Pool(processes) as p:
                p_fitpar_event = np.array(p.map(fit_pulse_shape, pulse_event[0, :, :]))
                l_fitpar_event = np.array(p.map(fit_pulse_shape, pulse_event[1, :, :]))
        else:
        ...
        p_stdevent_pulse, p_stdevent_fitpar = generate_standard_event(pulse_event[0, :, :],
                                                                      ...
                                                                      rise_time_intervall=[5, 100],
                                                                      decay_time_intervall=[50, 2500],
                                                                      onset_intervall=[1500, 3000],
                                                                      verb=True)
    ...
    if (-1.0 in tpa_list) and (0 in tpa_list):
        # ################# OPTIMUMFILTER #################
    ...
    if np.logical_and(tpa_list != -1.0, tpa_list != 0.0).any():
        ...
        with Pool(processes) as p:
            l_mainpar_list_tp = p.map(calc_main_parameters, pulse_tp[1, :, :])
        ...
        with Pool(processes) as p:
            l_fitpar_tp = np.array(p.map(fit_pulse_shape, pulse_tp[1, :, :]))
        ...
        testpulses.create_dataset('testpulseamplitute', data=np.array(metainfo_tp[0, :, 12]))
...
if __name__ == '__main__':
    gen_dataset_from_rdt(path='./data/run33_TUM40/',
                         fname='bck_056',
                         path_plots='./data/run33_TUM40/',
                         phonon_channel=6,
                         light_channel=7,
                         tpa_list=[0.0],
                         calc_fit=False,
