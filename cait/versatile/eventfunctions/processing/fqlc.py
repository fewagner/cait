...
class FQLC(FncBaseClass):
    ...
    def __call__(self, event):
        ...
        if self._method == "mmd":
            self._tmin1 = 600 + np.argmin(self._event_nobl[600:int(self._t_max)]) #start at 220 so we don't end up out of bounds when averaging later
