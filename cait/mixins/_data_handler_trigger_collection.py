...
class TriggerCollectionMixin:
    ...
    def trigger_coincidence(self,
            ...
            **Example:**
            ...
            coinc_flag = [[True]*len(event_ts)]
            for i, ts in enumerate(trigger_ts[1:]):
                inside, coinc_inds, outside = vai.timestamp_coincidence(event_ts, ts, interval)

                for k in range(i+1):
                    coinc_flag[k].extend([False]*len(outside))

                new_flag = np.array([False]*len(event_ts) + [True]*len(outside))
                new_flag[coinc_inds] = True
                coinc_flag.append(new_flag)

                new_ts = np.concatenate([event_ts, np.array(ts)[outside]])
                sortind = np.argsort(new_ts)
                event_ts = new_ts[sortind]

                coinc_flag[i] = np.array(coinc_flag[i])[sortind].tolist()
                coinc_flag[-1] = np.array(coinc_flag[-1])[sortind].tolist()

            # save final timestamps after event building and coincidence information
            self.set("triggers", timestamps=np.array(event_ts), dtype=np.int64, overwrite_existing=True)
            self.set("triggers", coincidences=np.array(coinc_flag), dtype=bool, overwrite_existing=True)
            
            # also save the original trigger timestamps exactly like the coincidence array
            original_ts = np.zeros(np.array(coinc_flag).shape, dtype=np.int64)
            for i, t in enumerate(trigger_ts):
                original_ts[i, np.array(coinc_flag[i])] = np.array(t)
                
            self.set("triggers", original_timestamps=original_ts, dtype=np.int64, overwrite_existing=True)
